...
- name: 项目名称
- version: 项目版本
- edition: 定义了项目使用的 Rust 的大版本，这点我觉得很棒，类似 npm package.json 中的 engines 字段
...
```json
{
  ...
  "version": "1.0.0",
  "engines": {
    "node": ">=18.0.0",  // 指定 Node.js 版本必须 ≥ 18.0.0
    "npm": ">=9.0.0"     // 可选：也可以指定 npm 版本
  }
}
```

跑题了 -，-，让我们回来继续看 Cargo.toml 的其他字段，有了项目基础信息，当然我们还要对依赖项进行管理，
...
在 `[dependencies]` 后则为项目依赖项，针对项的版本，源进行管理


<DividerSmall />


### 在 Rust 世界 第一个 `hello world`

终于到了最终环节，写一个 hello world 是所有程序员在学习一门新语言永远无法绕过的第一节课-，-，直接上代码
...
打开终端，运行 `cargo run` 我们就可以看到不同语言的 `你好世界` 被输出到bash上，恭喜 🎉！

现在让我们来看看这段代码，`fn main` 作为程序的默认入口，调用 `greet_world` 方法进行执行，由于 Rust 原生支持 UTF-8 编码，也就是说你可以很容易使用各国语言硬编码至程序中，而无需担心乱码问题。

`let` 关键词为我们默认创建了一个字符串切片来承接硬编码的内容，他的完整定义应该是 `let &str chinese = "世界，你好";`，同时不同于 `javascrpit`，在Rust中，字符串需要使用双引号来定义，单引号表示字符，末尾的 `;` 也有含义，表明这是一个完整的语句，
而不是返回一个表达式。

`println!` 是一个宏操作，后续会展开详细介绍这里。
